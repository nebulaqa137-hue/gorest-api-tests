 ...
 *
 * Dependency: cada paso depende del anterior (dependsOnMethods).
 */
@Epic("Integración")
...
public class UserIntegrationTests extends BaseTest {
    ...
    @Test(
        description  = "[Paso 1] POST — crear usuario nuevo",
        groups       = {"integration"}
    )
    ...
    // ─────────────────────────────────────────────
    // PASO 2 — GET y validar schema
    // ─────────────────────────────────────────────

    @Test(
        description     = "[Paso 2] GET — validar usuario creado + schema",
        groups          = {"integration"},
        dependsOnMethods = "step1_createUser"
    )
    @Story("Flujo completo")
    ...
    @Test(
        description      = "[Paso 3] PUT — actualizar nombre y status del usuario",
        groups           = {"integration"},
        dependsOnMethods = "step2_getAndValidateCreatedUser"
    )
    @Story("Flujo completo")
    @Severity(SeverityLevel.CRITICAL)
    ...
    // ─────────────────────────────────────────────
    // PASO 4 — Confirmar actualización con GET
    // ─────────────────────────────────────────────

    @Test(
        description      = "[Paso 4] GET — confirmar que la actualización persiste",
        groups           = {"integration"},
        dependsOnMethods = "step3_updateUser"
    )
    @Story("Flujo completo")
    ...
    public void step4_confirmUpdate() {
        ...
        .then()
                .statusCode(200)
                .body("status", equalTo("inactive"))     // confirmamos cambio
                .body("email",  equalTo(originalEmail))  // email no cambió
                .extract().response();
    ...
    @Test(
        description      = "[Paso 5] DELETE — eliminar el usuario creado",
        groups           = {"integration"},
        dependsOnMethods = "step4_confirmUpdate"
    )
    @Story("Flujo completo")
    @Severity(SeverityLevel.CRITICAL)
    ...
    // ─────────────────────────────────────────────
    // PASO 6 — Confirmar eliminación (404)
    // ─────────────────────────────────────────────

    @Test(
        description      = "[Paso 6] GET — confirmar que el usuario ya no existe (404)",
        groups           = {"integration"},
        dependsOnMethods = "step5_deleteUser"
    )
    @Story("Flujo completo")
    @Severity(SeverityLevel.CRITICAL)
